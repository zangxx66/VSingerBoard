...
def bilibili_container(page: ft.Page):
    ...
    async def on_save_click(e: ft.Event[ft.Button]):
        ...
        result = await async_worker.run_db_operation(
            db.add_or_update_bili_config(**data.__dict__)
        )
        if result > 0:
            ...
            content=ft.Row(
                ...
                controls=[
                    ...
                    ft.TextField(
                        label="房间号",
                        ref=room_id_text,
                        input_filter=ft.InputFilter(regex_string=r"^[1-9]\d*$"),
                    ),
                    ft.TextField(
                        label="粉丝牌等级",
                        ref=modal_lv_text,
    ...
    async def check_qr_code():
        ...
        while not qr_code_login.has_done():
            ...
            model_dic = {
                k: v for k, v in new_dic.items() if v is not None and k in model_fileds
            }
            ...
            if not cred:
                ...
                await async_worker.run_db_operation(
                    db.update_bcredential(pk=cred.id, **model_dic)
                )
    ...
    async def on_add_cred_click(e: ft.Event[ft.Button]):
        ...
        page.show_dialog(
            ft.AlertDialog(
                ...
                actions=[ft.Button("关闭", on_click=lambda ee: page.pop_dialog())],
            )
        )
            ...
            content=ft.Column(
                controls=[
                    ft.Button(
                        ...
                        content="新建账号",
                        bgcolor=ft.Colors.PRIMARY_FIXED_DIM,
                        on_click=on_add_cred_click,
                    ),
                    ft.Text(
                        value="未登录账号无法获取到弹幕用户昵称等信息，如有需要可添加一个小号",
                        size=20,
    ...
    def generate_columns():
        ...
        return [
            ...
            ftd.DataColumn2(
                label="uid", heading_row_alignment=ft.MainAxisAlignment.START
            ),
                            ...
                            content=ft.Row(
                                controls=[
                                    ft.Button(
                                        ...
                                        data={"id": item.id, "enable": not item.enable},
    ...
    async def on_status_click(e: ft.Event[ft.Button]):
        ...
        result = await async_worker.run_db_operation(
            db.update_bcredential(pk=id, enable=enable)
        )
        if result > 0:
            ModernToast.success(page, "保存成功")
            async_worker.submit(bili_manager.restart())
            credential_list = await async_worker.run_db_operation(
                db.get_bcredential_list()
            )
